...
The durable benchmark needs a real exception. In both of its modes it copies
writes and messages into host-owned batches, performs its barrier outside
`Effects`, and never calls `confirmWritesDurable`. The grouped mode also
keeps the message queue private until one shared barrier completes. This host
owns the safety obligation outside `Effects` in both modes, so the whole
fixture moves to the exception namespace.
...
#table(
  ...
  [`src/replicated_log.zig`, `src/learner.zig`, `src/bit_set.zig`],
  [Convert compile-time option assertions to clear compiler errors. Do not
    change runtime behavior. `ReplicatedLog` always builds on the enforced
    factory and gets no host-managed variant.],
...
Existing gates keep their safety expectations. The seeded fault simulation
and the reconfiguration harness run under `zig build test`. The zaxonlite
package runs its own crash, cluster, role, fault-network, and C ABI suites
from its directory. The path-dependency integration consumer, previously an
orphan step, now runs under `zig build test`. The conformance appendix is
prose kept in sync with the handlers, not a build step.
