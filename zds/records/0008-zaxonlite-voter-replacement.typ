...
The cluster must contain at least three voters. The sealed configuration's
read quorum must be satisfiable by the survivors alone, because the removed
voter may be gone and the replacement never counts. With two voters that
quorum is unreachable, so the request is rejected during validation.
...
- Snapshot installation and checkpoint-proof validation use the local
  static member array, not the decided stop sign.
...
This split is already present in the code. Rollover initializes the next
consensus membership from the chosen stop sign, but the host keeps its
startup member array. After a changed-member rollover, snapshot installation
and proof validation would read the stale array. A node would reject its own
checkpoint proof. The decided registry closes that gap.
...
A new module, `zaxonlite/src/registry.zig`, owns the decided registry. The
existing `configuration.zig` keeps its current job, loading config files
and secrets; it gains only the admin allow-list described under security.
...
flags cannot override it. A conflicting flag is a startup error, exactly as
the identity file already rejects a changed node ID or role.
...
derived from the replacement member set. The bootstrap derivation runs only
when no registry file exists. A present but unreadable registry stops the
server; it never falls back to derivation.

The registry lives beside the existing `identity` and `CURRENT` files:

```text
registries/<configuration id, 16 hex>   canonical registry blob
REGISTRY                                16-hex pointer to the active blob
PENDING-OP                              the one in-flight replacement request
```

The blob directory is deliberately not named `registry`: common macOS and
Windows filesystems are case-insensitive, so that name would collide with
the `REGISTRY` pointer file.

All three use the node's existing atomic write primitive: write a temporary
file, sync it, rename it into place, sync the pathname transition. The
`REGISTRY` pointer has the same shape and the same strict length check as
`CURRENT`. It is a separate file because the two pointers advance at
different points in the rollover order. Superseded blobs are garbage
collected with the same retention discipline as old snapshots.
...
The metadata tag moves from `zx1` to `zx2`, and the host raises its
stop-metadata capacity from 256 to 512 bytes. The registry digest is now
part of the safety boundary. For a replacement, `zx2` also carries a
bounded reconstruction seed: the operation ID, the old node ID, the new
node ID, and the new endpoint.

The seed makes the next registry a pure function of the current registry
and the chosen stop sign. Every survivor rebuilds the candidate registry
locally, verifies its digest against the stop metadata, and never needs
the network to activate. Only the replacement node, which holds no
predecessor registry, fetches the blob from a survivor and verifies the
same digest. The endpoint length cap at request validation keeps the seed
inside the metadata bound.
...
The server keeps client TCP connections open. The `Node` itself completes
the rollover in place, exactly as it does for a same-member checkpoint
today. What the server rebuilds is everything around it: the member table,
the peer senders, and the peer admission rules, all derived from the
decided registry.

The client connection layer must not retain a pointer to the old member
table or sender set. Each request resolves the current in-memory generation
before dispatch. An in-flight read either completes before quiescence or
...
The durable `REGISTRY` pointer is the recovery authority for membership. It
is written after the snapshot pointer and before the identity file,
extending the rollover order the node already uses. A crash before its
update restarts the old configuration. A crash after its update restarts
...
and the required tail. The registry travels over a new fetch message pair
and is checked against the digest bound in the enrollment response. The
replacement verifies every digest and the sealed quorum. It then installs
the state durably and starts in the next configuration.
...
#block(width: 100%, breakable: false)[
  #table(
    ...
    [Retain the stop-sign algorithm. Add changed-member rollover coverage.
      Expose the decided stop slot, a by-reference stop-sign accessor, and
      public member validation for host decoders.],
    [`zaxonlite/src/registry.zig` (new)],
    [Define canonical decided-registry encoding, validation, hashing, durable
      ...
      precedence. `configuration.zig` keeps only config and secret loading.],
    ...
    [Add CLI request, orthogonal quorum and installation fields, structured
      errors, and leader hints. `membership status` is the CLI's first
      nested subcommand.],
    ...
    [Not edited. The amendments section of this record supersedes the
      affected clauses, so their section numbers and conformance anchors
      stay valid.],
    [`specs/`],
    [Add the bounded host-level replacement model beside `Paxos.tla`, its
      TLC configuration, and a second action-to-code map in
      `specs/README.md`.],
    [`docs/zaxonlite/book`],
    [Update the front matter, learning guide, quickstart, CLI, product,
      storage, cluster, embedding, C ABI, client and gateway, examples,
      operations, formats, verification, reference, and conformance
      chapters. Reword the database-identity derivation as bootstrap-only
      wherever it appears. Keep embedded-node membership explicitly fixed.],
    [`zaxonlite/README.md`, `README.md`],
    [Replace the static-membership and no-replacement limits once the
      feature lands, and record the new operator commands.],
  )
]
...
Add a bounded host-level model, `specs/VoterReplacement.tla`, beside
`specs/Paxos.tla` with:
...
Administrator identity is a certificate, not a node. Admin certificates use
a distinct common-name namespace, `zaxon-admin-<name>`, issued by the same
cluster authority as node certificates. The server holds an explicit
allow-list of admin names; an empty list denies every privileged request. A
node certificate can never authorize a membership operation. PSK
connections carry no certificate at all, so the operation is structurally
unreachable in development mode.
...
2. Land proof v2 and validate same-member checkpoints through the new
   verifier.
3. Extend the existing changed-member `ReplicatedLog` simulation and add
   the deterministic host schedule.
...
- The decided registry lives in a new `registry.zig` module with its own
  `REGISTRY` pointer file beside `CURRENT`, reusing the node's atomic write
  primitive and recovery ladder.
- Survivors reconstruct the next registry deterministically from `zx2` stop
  metadata. Only the replacement fetches the blob, and it verifies the
  decided digest.
...
are not edited. Their section numbers, and the conformance clauses anchored
to them, stay valid. Where a clause below and this record disagree, this
record wins once it reaches the `committed` state.

== Amendments to ZDS 0002

The non-goal of automatic voter replacement stands; this record adds an
operator-initiated operation. ZDS 0002 already names the safe form in its
rqlite review: stop-sign epoch transition, learner catch-up proof, and
operational fencing. That row, and the roadmap paragraph on runtime voter
reconfiguration, are superseded for the one-for-one replacement case. The
reserved `members add` and `members remove` commands are not implemented;
the operator surface is `replace-voter` and `membership status`. The TLA+
obligation ZDS 0002 records for dynamic reconfiguration is discharged by
the model in this record.

== Amendments to ZDS 0003

The statements that membership stays operator-configured and static, that
eviction is an offline procedure, and that a membership-changing snapshot
is rejected by the transfer path are superseded for this operation. The
forward-looking clause of that record is now in force: a decided stop sign
with a new member set atomically replaces the transport allowlist and
closes sockets for removed nodes, and consensus membership overrides
certificate validity. Its open question, which quorum attests a
membership-changing snapshot, is answered: a read quorum of the sealed
voter set. The security matrix gains one row: the privileged replacement
request, authenticated by an admin certificate against an explicit
allow-list, absent from development PSK mode. Enrollment now authorizes a
decided next-registry identity after the stop is chosen; it still never
chooses membership itself.

== Amendments to ZDS 0004

The release limit of one through nine static voters is amended: the voter
count stays fixed, and one data voter can be replaced through the decided
operation in this record. Enrollment's scope is amended the same way: it
remains a narrow bootstrap that grants no role, but the registry it checks
is the decided registry, not the startup flags. The identity section gains
...
- `zaxonlite/src/checkpoint_proof.zig` - checkpoint proof encoding and quorum
  confirmation
- `zaxonlite/src/registry.zig` - decided registry, fence, and operation
  ring (new module)
...
- `docs/zds/records/0003-zaxonlite-security-remediation-plan.typ` - trust
  model, enrollment, and eviction rules amended above
- `docs/zds/records/0004-zaxonlite-format.typ` - persisted and wire
  format registry
- `docs/zds/records/0006-windows-durability.typ` - the amendment pattern
